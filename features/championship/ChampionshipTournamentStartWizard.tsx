...
export const ChampionshipTournamentStartWizard: React.FC<ChampionshipTournamentStartWizardProps> = ({
  ...
  React.useEffect(() => {
    if (selectedTemplate) {
      ...
      // Default deadlines for each stage (use template deadline/eventDate if present, else estimate)
      const deadlines: Record<string, string> = {};
      const stages = selectedTemplate.stages || [];
      stages.forEach((st, idx) => {
        if (st.eventDate) {
          deadlines[st.id] = st.eventDate;
        } else if (st.deadlineDate) {
          deadlines[st.id] = st.deadlineDate;
        } else {
          const d = new Date();
  ...
  const handleFinishWizard = async () => {
    ...
    try {
      ...
      // Auto-lock template to ensure tamper-proof historical rules
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] text-slate-500 font-medium">
                          {stage.type === 'finals_day' || stage.isFinalsDay ? 'Datum:' : 'Zu spielen bis:'}
                        </span>
                        <input
                          type="date"
